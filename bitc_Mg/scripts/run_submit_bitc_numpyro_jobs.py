...
parser.add_argument( "--experimental_design_parameters_dir",  type=str, default="1.experimental_information")
...
parser.add_argument( "--script",                        type=str, default="/home/vla/python/bitc_Mg/scripts/run_numpyro.py")
...
cd ''' + out_dir + '''\n''' + \
    ...
    exper_list = str(exper_names).replace("[","").replace("]","").replace("'","").replace(",","")
    print(exper_list)
        
    qsub_file = os.path.join(out_dir, "numpyro.job")
    log_file  = os.path.join(out_dir, "numpyro.log")
